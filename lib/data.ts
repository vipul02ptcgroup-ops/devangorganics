...
const defaultRawProducts: RawProductRecord[] = [
  {
    "id": 160,
    "name": "Wild Cordyceps",
    "sku": null,
...
function normalizeCategoryName(input?: string | null) {
  ...
  if (value === "herb tea" || value === "organic tea" || value === "tea") {
    return "Herb Tea";
  }
  if (value === "natural soaps" || value === "soap" || value === "soaps") {
    return "Natural Soaps";
  }
  if (value === "pickles" || value === "pickle") {
    return "Pickles";
  }
  return input.trim();
}
...
export function normalizeProductRecord(
  ...
  return {
    ...
    badge: index % 3 === 0 ? badges[index % badges.length] : undefined,
...
export function serializeProductToRawRecord(product: Product): RawProductRecord {
  return {
    id: product.id,
    name: product.name,
    sku: null,
