      ...
      <div className="flex items-center justify-between h-[72px] px-[5%]">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          {isAdmin ? (
            <Link
              href="/admin"
              className="text-[#C9A84C] text-[0.82rem] font-medium tracking-wide hover:text-[#E8C96A] transition"
            >
              Admin Panel
            </Link>
          ) : null}
        ...
        <div className="bg-[#111111] border-t border-[#2A2A2A] px-[5%] py-4 md:hidden">
          ...
          {isAdmin ? (
            <Link
              href="/admin"
              onClick={() => setMobileOpen(false)}
              className="block py-3 text-[#C9A84C] text-sm border-b border-[#1A1A1A]"
            >
              Admin Panel
            </Link>
          ) : null}
